...
public static unsafe partial class Sdl {
    ...
    /// <param name="camera">opened camera device.</param>
    /// <remarks>
    /// <para><strong>Thread Safety</strong>: It is safe to call this function from any thread, but no thread may reference device once this function is called.</para>
    /// <para><strong>Version</strong>: This function is available since SDL 3.2.0.</para>
    ...
    /// </remarks>
    /// <returns>Returns the name of the camera driver at the requested index, or <see langword="null" /> if an invalid index was specified.</returns>
    ...
    /// <seealso cref="OpenCamera" />
    /// </remarks>
    /// <returns>Returns the instance ID of the specified camera on success or 0 on failure; call <see cref="GetError()" /> for more information.</returns>
    ...
    /// <remarks>
    /// <para>Cameras will not function between when the device is opened by the app and when the user permits access to the hardware.</para>
    /// <para>On some platforms, this presents as a popup dialog where the user has to explicitly approve access; on others the approval might be implicit and not alert the user at all.</para>
    /// <para><strong>Thread Safety</strong>: It is safe to call this function from any thread.</para>
    /// <para><strong>Version</strong>: This function is available since SDL 3.2.0.</para>
    /// <seealso cref="OpenCamera" />
    /// <seealso cref="CloseCamera" />
    /// </remarks>
    /// <returns>Returns -1 if user denied access to the camera, 1 if user approved access, 0 if no decision has been made yet.</returns>
    ...
    /// <param name="instanceId">the camera device instance ID.</param>
    /// <remarks>
    /// Most platforms will report <see cref="CameraPosition.Unknown" />, but mobile devices, like phones,
    /// can often make a distinction between cameras on the front of the device (that points towards the user, for taking &quot;selfies&quot;)
    /// and cameras on the back (for filming in the direction the user is facing).
    /// <para><strong>Thread Safety</strong>: It is safe to call this function from any thread.</para>
    /// <para><strong>Version</strong>: This function is available since SDL 3.2.0.</para>
    /// <seealso cref="GetCameras" />
    /// </remarks>
    ...
    public static CameraPosition GetCameraPosition(uint instanceId) {
        ...
        if (!Enum.IsDefined(position)) {
            LogError(LogCategory.Error, $"Invalid camera position value '{position}' for instance ID {instanceId}.");
        }
    ...
    /// </remarks>
    /// <returns>
    /// <para>(SDL_CameraID *) Returns a 0 terminated array of camera instance IDs or <see langword="null" /> on failure; call <see cref="GetError()" /> for more information.</para>
    ...
    /// <remarks>
    /// <para>This returns a list of all formats and frame sizes that a specific camera can offer.</para>
    /// <para>This is useful if your app can accept a variety of image formats and sizes and so want to find the optimal spec that doesn't require conversion.</para>
    /// <para><strong>Thread Safety</strong>: It is safe to call this function from any thread.</para>
    /// <para><strong>Version</strong>: This function is available since SDL 3.2.0.</para>
    /// <seealso cref="GetCameras" />
    /// <seealso cref="OpenCamera" />
    /// </remarks>
    /// <returns>
    /// <para>(SDL_CameraSpec **) Returns a <see langword="null" /> terminated array of pointers to <see cref="CameraSpec"/> or <see langword="null" /> on failure; call <see cref="GetError()" /> for more information.</para>
    /// <para>This is a single allocation that should be freed with <see cref="Free" /> when it is no longer needed.</para>
    /// </returns>
    ...
    /// <remarks>
    /// <para>The names of drivers are all simple, low-ASCII identifiers, like &quot;v4l2&quot;, &quot;coremedia&quot; or &quot;android&quot;.</para>
    ...
    /// <para>For example, a build of SDL might have v4l2 support, but if there's no kernel support available, SDL's v4l2 driver would fail if used.</para>
    /// <para><strong>Thread Safety</strong>: It is safe to call this function from any thread.</para>
    /// <para><strong>Version</strong>: This function is available since SDL 3.2.0.</para>
    ...
    /// <remarks>
    /// <para>You can open the device with any reasonable spec, and if the hardware can't directly support it, it will convert data seamlessly to the requested format.</para>
    /// <para>This might incur overhead, including scaling of image data.</para>
    /// <para><strong>Thread Safety</strong>: It is safe to call this function from any thread.</para>
    /// <para><strong>Version</strong>: This function is available since SDL 3.2.0.</para>
    /// <seealso cref="GetCameras" />
